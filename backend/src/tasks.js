...
router.get('/guest/:id',async(req,res)=>{
    try{
        let{id}=req.params
        let todo=await taskModel.findOne({_id:id })
    ...
    catch(e){res.status(400).send({error:'Task is not exist'})}
})
...
//                    
        ...
        router.delete('/alltodos',authMe,async(req,res)=>{
            try{
                    let tasks=await taskModel.find({owner:req.user._id})
                    console.log(tasks)
                    if(!tasks)  throw new Error ('There is no Todos for that user')
                    else  {
                         let todos=await taskModel.deleteMany({owner:req.user._id})
    //                    
                         console.log(todos)
                        res.status(200).send({
                            status:true,
                            data:todos,
                            message:'tasks has been removed'
                        })
                    }
